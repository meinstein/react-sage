...
test('useMutation', async () => {
  ...
  const { result: r, waitForNextUpdate } = renderHook(() => useMutation(mockedApiClientMethod, onSuccess, onError))
  ...
  expect(r.current.result).toEqual({ error: null, loading: true, response: undefined })
   ...
   */
  expect(r.current.result).toEqual({
    error: null,
    loading: true,
    response: undefined
  })
